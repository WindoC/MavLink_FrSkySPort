...
--Init Variables
	...
	local cmin = 0
	local number_cells = 0
	local vfas = 0
	local watts = 0
	local batteryremainps = 0
...
-- draw Wh Gauge
	local function drawWhGauge()
	   ...
	   lcd.drawFilledRectangle(75,9,9, ( ( 100 - batteryremainps ) / 100 * 55 ) + 0, 0)
	end
...
-- play alarm wh reach maximum level
	local function playMaxWhReached()
	  ...
	  elseif batteryremainps < 11 then
	    if localtimetwo >=1600 then --16s
		  playFile("/SOUNDS/en/TELEM/ALARM3K.wav")
		  localtimetwo = 0
		end
      elseif batteryremainps < 21 then
	    if localtimetwo >=3000 then --30s
		  playFile("/SOUNDS/en/TELEM/ALARM3K.wav")
		  localtimetwo = 0
		end
	  end
